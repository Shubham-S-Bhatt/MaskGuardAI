...
#include "lcd16x2.h"  // For LCD screen
...
int main(void)
{
  ...
  lcd16x2_init_4bits(LCD_RS_GPIO_Port, LCD_RS_Pin, LCD_E_Pin,
		  	  	  	 LCD_D4_GPIO_Port, LCD_D4_Pin, LCD_D5_Pin, LCD_D6_Pin, LCD_D7_Pin);   // LCD initialize

  for (int i = 0; i <= 16; i++)                               // "PHYSICS PROJECT" scrolling right
  {
	  lcd16x2_setCursor(0,0);
	  lcd16x2_printf("MINI PROJECT-2B");
	  lcd16x2_shiftRight(1);
	  HAL_Delay(200);
  }


  lcd16x2_setCursor(0,1);
  lcd16x2_printf("GROUP: 1");
  HAL_Delay(1000);
